function [lambda,x,iter] = invpowershift_it(A,mu,tol,nmax,x0)
% 
% [lambdaVect, x, iter] = invpowershift_it(A, shift, toll, nmax, x0)
%
...
% shift     Valore di shift
% toll      Tolleranza sul criterio d'arresto (differenza iterate relativa)
%           (valore default 1e-6) 
...
%
% Parametri di uscita:
% lambdaVect    Approssimazione dell'autovalore di A piu' prossimo a shift,
%               come iterate, dimensione: iter + 1 (vettore riga)
% x             Approssimazione dell'autovettore corrispondente a lambda
%               (non normalizzato)
...
while (err>tol*abs(lambda(end))) && (abs(lambda(end))~=0) && (iter<nmax)
